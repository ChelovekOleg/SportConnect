...
if (isset($_POST['e_mail']) and isset($_POST['password'])){
    ...
    $query="SELECT * FROM sport.users where email = '".$_POST['e_mail']."';";
    $myProfile=$mysqli->query($query);
    $myProfileData = mysqli_fetch_array($myProfile);
//
//    $result = mysqli_query($connection, $query) or die(mysqli_error($connection));
//    $count = mysqli_num_rows($result);
//var_dump($myProfileData['id']); die();
    if ($myProfileData['email'] ==  $_POST['e_mail'] && $myProfileData['password'] == $_POST['password']){
//            $_SESSION['e_mail'] = $_POST['e_mail'];
            header('Location: home.php?uid='.$myProfileData[0]['email'].'' );
...
<div class="container">
    <form class="form-signin" method="POST" action="login.php">
        ...
        <img src="sportlogo.png" class="img-fluid" alt="Responsive image">
